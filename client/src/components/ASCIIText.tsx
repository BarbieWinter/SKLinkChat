...
class AsciiFilter {
  ...
  reset() {
    if (this.context) {
      ...
      this.pre.style.backgroundAttachment = 'local' // 'fixed' would use viewport coords → differs per window size
  ...
  render(scene: THREE.Scene, camera: THREE.Camera) {
    ...
    if (this.context) {
      ...
      // hue-rotate removed — colour is now deterministic via the CSS gradient
    }
  }
    ...
    if (w && h) {
      ...
      for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
          ...
          // Skip fully transparent AND anti-aliased edge pixels (< 72).
          // Without this threshold, font edge pixels (alpha 1–60) map to
          // characters and "leak" outside the intended glyph shapes.
          if (a < 72) {
            str += ' '
            continue
          }
...
class CanvAscii {
  ...
  constructor(
    ...
    this.camera.position.z = 30 // updated after mesh is created in setSize()
  ...
  async init() {
    try {
      ...
      // Continue with browser fallback fonts if custom font loading fails.
    }
  ...
  setSize(w: number, h: number) {
    ...
    // Recalculate camera Z so the full plane stays visible after any resize.
    // tan(halfFOV) for the 45° vertical FOV camera.
    ...
    // Keep pre clipping bounds in sync with the new container size
    if (this.filter.pre) {
      this.filter.pre.style.width = `${w}px`
      this.filter.pre.style.height = `${h}px`
    }
  }
  ...
  updateRotation() {
    // Range reduced from ±0.5 to ±0.1 rad — subtle tilt, flat when mouse at rest,
    // consistent starting state across browsers.
    const x = map(this.mouse.y, 0, this.height, 0.1, -0.1)
    const y = map(this.mouse.x, 0, this.width, -0.1, 0.1)
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    // Attach a ResizeObserver so the instance always stays in sync with the
    // container — regardless of which initialisation path was taken.
    ...
    const setup = async () => {
            ...
            if (entry.isIntersecting && entry.boundingClientRect.width > 0 && entry.boundingClientRect.height > 0) {
              ...
              if (!cancelled) {
                ...
                if (!cancelled && asciiRef.current) {
                  ...
                  // Re-sync: container may have changed size during async init
                  const cur = containerRef.current!.getBoundingClientRect()
                  if (Math.abs(cur.width - w) > 1 || Math.abs(cur.height - h) > 1) {
                    asciiRef.current.setSize(cur.width, cur.height)
                  }
                  attachResizeObserver(containerRef.current!)
                }
              }
            }
          },
      ...
      if (!cancelled && asciiRef.current) {
        ...
        // Re-sync: container may have changed size during async font loading
        const cur = containerRef.current!.getBoundingClientRect()
        if (Math.abs(cur.width - width) > 1 || Math.abs(cur.height - height) > 1) {
          asciiRef.current.setSize(cur.width, cur.height)
        }
